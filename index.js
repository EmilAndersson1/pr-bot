...
const processingReactions = new Set();
// Store bot's own ID
let myBotId = null;
...
----------------------------- */
function formatComplexity(level) {
...
----------------------------- */
function getAuthorIdFromText(text) {
...
app.event("reaction_added", async ({ event, client, logger }) => {
  try {
    ...
    try {
      ...
      ------------------------------------------- */
      if (event.reaction === "speech_balloon") {
        const text = authorId
          ? `<@${authorId}>, someone made a comment or two on your PR.`
          : `Someone made a comment or two on this PR.`;
      ...
      if (event.reaction === "white_check_mark") {
        const text = authorId
          ? `<@${authorId}>, your PR has been approved, time to ship it!`
          : `This PR has been approved, time to ship it!`;
      ...
      if (event.reaction === "merged") {
        ...
        if (authorId) {
          warnText = `<@${authorId}> heads up – this PR entry will be deleted in 30 seconds. Remove the :merged: reaction to cancel.`;
        }
        ...
        const timeoutId = setTimeout(async () => {
          try {
            ...
            const stillHasMerged = reactions.some((r) => r.name === "merged");
        ...
        // Note: we don't remove from processingReactions immediately if we want to track it?
        // Actually, 'processing' is just for the window between event trigger and timer set.
        // If we set the timer, we rely on 'deletionTimers'.
...
app.event("reaction_removed", async ({ event, client, logger }) => {
  try {
    if (event.reaction !== "merged") return;
    if (event.item.type !== "message") return;
